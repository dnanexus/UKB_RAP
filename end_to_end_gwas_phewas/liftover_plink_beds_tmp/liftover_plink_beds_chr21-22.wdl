...
task merge_and_split_by_chr {
  ...
  command <<<
    ...
    ## Get autosomal (chr21-22) PLINK file set
    plink2 --bfile "ukb_~{reference}_merged" --chr 21-22 --make-bed \
      --out "autosomal_dir/ukb_c21-22_~{reference}_merged"  \
      --allow-extra-chr --threads $(nproc) --memory "${memXmx_m}"
    cp "ukb_~{reference}_merged.fam" "autosomal_dir/ukb_c21-22_~{reference}_merged.fam"
...
#    cp "ukb_~{reference}_merged.fam" "ukb_cPAR_~{reference}.fam"
    ...
    plink2 --bfile "ukb_~{reference}_merged" --not-chr 21-22 --make-bed \
  ...
  output {
    ...
    Array[File] autosomal_plinks = glob("autosomal_dir/ukb_c21-22_~{reference}_merged.*")

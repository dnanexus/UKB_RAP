...
Following code assumes that you have `age_month_birth.csv` file in the root directory of your current UKB RAP project. You can create this file by using Table exporter.

```{r}
system('dx download age_month_birth.csv')
```
...
```{r}
data <- read.csv('age_month_birth.csv')
head(data)
```
